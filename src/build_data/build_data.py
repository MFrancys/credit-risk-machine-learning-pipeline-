...
from previous_internal_application_features.make_features import (
    build_previous_internal_app_features,
)
...
def go(args):
    ...
    df["loan_origination_datetime_month"] = df["LOAN_ORIGINATION_DATETIME"].dt.strftime(
        "%Y-%m"
    )

    credit_report_df = pd.read_parquet(args.credit_report_dataset_path_artifact)
    credit_report_df = pd.merge(
        credit_report_df,
        df[["LOAN_ORIGINATION_DATETIME", "customer_id", "APPLICATION_DATETIME"]],
        how="left",
        on="customer_id",
    )
    ...
    df_credit_reports_features["credit_reports__loans_count"] = (
        df_credit_reports_features["credit_reports__loans_count"].fillna(0)
    )
    ...
    final_df = pd.merge(
        final_df, df_credit_reports_features, how="left", on="customer_id"
    )
    final_df = final_df.fillna(0)

    formatted_date = datetime.now().strftime("%Y%m%d")
    store_data_path = (
        Path.cwd().parent.parent / f"models/{formatted_date}_final_dataset.pickle"
    )
