...
def build_aggregate_credit_report_information_by(
    df: pd.DataFrame, aggregate_by: str
) -> pd.DataFrame:
    """
    ...
    df_pivot = df_aggregates.pivot_table(
        index="customer_id", columns=aggregate_by, values=values, aggfunc="first"
    )

    features = [
        "credit_reports__" + "_".join(col).lower() for col in df_pivot.columns.values
    ]
...
def build_aggregate_credit_report_information(
    df: pd.DataFrame, aggregate_column_names: str = "credit_reports__"
) -> pd.DataFrame:
    """
    ...
    df_aggregates.columns = [
        aggregate_column_names + "_".join(i) for i in df_aggregates.columns.values
    ]
    df_aggregates = df_aggregates.reset_index()
...
def build_credit_report_features(df_aux: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df.assign(
        age=np.where(
            df["loan_opening_date"].isnull(),
            np.nan,
            np.where(
                df["loan_closing_date"].isnull(),
                (df["application_datetime"] - df["loan_opening_date"]).dt.days,
        ...
        is_opening=np.where(
            df["loan_closing_date"].isnull(),
            1,
            np.where(~df["loan_closing_date"].isnull(), 0, np.nan),
        ),
        debt_ratio=(df["current_balance"] / df["max_credit"]).replace(
            [np.inf, -np.inf], np.nan
        ),
        severity_delayed_payments=(
            df["delayed_payments"] / df["total_payments"]
        ).replace([np.inf, -np.inf], np.nan),
        balance_due_ratio=(df["balance_due"] / df["max_credit"]).replace(
            [np.inf, -np.inf], np.nan
        ),
        balance_due_worst_delay_ratio=(
            df["balance_due_worst_delay"] / df["max_credit"]
        ).replace([np.inf, -np.inf], np.nan),
        has_delayed_payments=np.where(
            df["delayed_payments"] > 0,
            1,
            np.where(df["delayed_payments"] == 0, 0, np.nan),
        ),
        is_individual_responsibility=np.where(
            df["responsability_type"] == "INDIVIDUAL (TITULAR)",
            1,
            np.where(~df["responsability_type"].isnull(), 0, np.nan),
        ),
    )

    agg_df = build_aggregate_credit_report_information(df).rename(
        columns={
    ...
    agg_df_open_loans = build_aggregate_credit_report_information(
        df_aux, aggregate_column_names="credit_reports__open_loans_"
    ).rename(
        columns={
    ...
    agg_df_by_credit_type = build_aggregate_credit_report_information_by(
        df, aggregate_by="account_type"
    )
    ...
    df_pivot = df_pivot.assign(
        credit_reports__opening_loans_ratio=df_pivot["credit_reports__open_loans_count"]
        ...
        / df_pivot["credit_reports__loans_count"],
        credit_reports__debt_ratio=df_pivot["credit_reports__balance_due_sum"]
        / df_pivot["credit_reports__max_credit_sum"],
        credit_reports__debt_due_ratio=df_pivot["credit_reports__balance_due_sum"]
        / df_pivot["credit_reports__balance_due_sum"],
    )
